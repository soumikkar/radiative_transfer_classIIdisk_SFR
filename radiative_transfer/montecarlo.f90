...
subroutine do_monte_carlo(nphot, spectrum, tdust, scatsrc, &
 ...
 ! real(8) :: cumulener(:,:,:,:), cumulener_bk(:,:,:,:)
 ! real(8) :: freqdistr(:,:,:)
  !integer :: ilastphot(:,:), iphotcount(:,:)


  ! Allocate arrays based on known frequencies and grid sizes
  !allocate(starlum(freq_nr), extlum(freq_nr))
  !allocate(integerspec(freq_nr, irsi_frsizey))
...
!print*,'imirt=', imirt
 ...
 radbnd_starlumtot = 0.d0
  ...
  !print*, "spectrum calculation=", spectrum(inu,it)
  ! Compute dust temperatures
  !call update_dust_temperatures(tdust, cumulener, vol, ntemp)
...
!print*, "tdust", tdust(isize, ispec, it, ir)
  ! Clean up
  !deallocate(starlum, extlum, integerspec)
...
subroutine randomfreq(flux, inupick, iseed)
  !use iso_fortran_env, only: dp => real64
 ! use parameters_mod, only: freq_nr, FRSIZE_FREQ
  !use common_dust_mod, only: dust_freq_wgt
  implicit none
...
subroutine make_emiss_dbase()
  ...
  ! Arguments
  !integer, intent(in) :: dust_nr_species, db_ntemp
  !integer, intent(in) :: dust_nr_size(:)
  !double precision, intent(in) :: freq_nu(:)
  !double precision, intent(in) :: kappa_a(:,:,:)
  !double precision, intent(in) :: db_temp(:)

  !double precision, intent(inout) :: db_enertemp(:,:,:)
  !double precision, intent(inout) :: db_emiss(:,:,:,:)
  !double precision, intent(inout) :: db_cumulnorm(:,:,:,:)
  ...
  ! Initialize
  !freq_nr = size(freq_nu)
 ! allocate(cellalpha(freq_nr), fnu_old(freq_nr), fnu_diff(freq_nr))
!  allocate(diffemis(freq_nr), cumul(freq_nr+1))
...
subroutine do_absorption_event(vol, rhodust, kappa_a, alpha_a, tdust, &
  ...
  !use constants_module      ! contains pi, twopi, pihalf, fourpi
  !use dust_properties_module
  !use montecarlo_utilities
  implicit none
 ...
 ! external :: absevfunc
  ...
  ! Disable quantum heating for remainder of this photon's path (if applicable)
  !if (iqactive == 1) iqactive = 0
...
!#endif
    ...
    rn = ran2(iseed)
    !if (rn <= radbnd_starlumtot/lumtot) then
        ! Photon from star
        if (rstar == 0.0d0) then
            mu = 1.0d0
            phi = 0.0d0
...
subroutine walk_cells_twodee(inu, r, theta, mu, phi, &
                            ...
                            energy, imethod, stot, &
                            idestroy)
  !use montecarlo_constants, only: dp, pi, pihalf, twopi
  implicit none
...
!#endif
  ...
  ! other locals as needed...

  ! Initialize and checks omitted for brevity
  ! ...
imirt = 1
...
!call find_next_r_crossing(r, mu, r0, r1, r_r, mu_r, ilr_r)
!call find_next_theta_crossing(r, theta, mu, phi, t0, t1, s, s_th, th_th, ilr_th)
  ...
  do is = 1, 2*nr + 2*nt + 2
    ...
    !if (ir1 < 1 .or. ir1 > nr+1) stop 'Radial index out of bounds'
   ! if (it1 < 1 .or. it1 > nt+1) stop 'Theta index out of bounds'
...
SUBROUTINE pick_randomfreq_db(temp, enerpart, inupick, iseed)
 ...
 ! USE mc_spec_module
  ...
  !use numerical_receipe_subroutines
