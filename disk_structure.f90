...
contains
    ...
    subroutine simpledisk_vertstruct(rstar_in,tstar_in,mstar_in, &
                                     ...
                                     ifast_in,dostr_in)
        !
        ! Fortran 90 version of the IDL program to set up and run a
        ! circumstellar disk model simulation.
 ...
 ifast_in
        ...
        !
        ! (Following the structure of the IDL program)
        ...
        !
        ! Smooth the inner rim if necessary.
       ! if (present(drsm_in)) then
           ! call smooth_rim(r_val, theta_val, rhodust_in, kappa, &
                    !        sigdust=sigdust1_in, drsm=drsm_in, tautol=0.5d0)
        !end if
        ...
        !b_struct%r = r_val
        !b_struct%theta = theta_val
        !b_struct%rho = rhodust_in
        !tt_struct = maketau(b_struct, kappa)

        !
        ! Write a message
        !ddr = r_val(nr) / r_val(nr-1) - 1.0d0
        !write(*,*) 'TAUR = ', tt_struct%taur(nr, nt)
        !write(*,*) 'DR/R = ', ddr

        !
        ! Check the optical depth at the inner edge
       ! tr0 = tt_struct%taut(1, nt)
        !if (tr0 > 1.0d0) then
        !    write(*,*) 'Optical depth error!'
        !    write(*,*) 'Inner grid cell at equator optically thick'
        !    write(*,*) 'Is this okay?'
        !    write(*,*) '(type 1)'
        !    read(*,*) i_val
        !    if (i_val /= 1) then
       ...
       ! end if
        ...
        !
        ! The rest of the calls to subroutines would be here...
