module disk_radmc_subroutines
    ...
    ! [cite_start]We assume that the 'erf' function is available from an external library [cite: 2]
    ! or is a user-defined function. [cite: 3]
    !interface
    ...
    ! [cite_start]A derived type to represent the opacity data structure from the readopac function. [cite: 4]
    ...
    ! [cite_start]A derived type to represent the disk data structure. [cite: 6]
    type disk_data_type
...
contains
    ...
    function find_peak_starspec(nu, nulnu) result(inu)
     ...
     ! [cite_start]Finds the index of the peak value in a spectrum. [cite: 7]
    ...
    function findchi(kappa, alpha, sigma) result(chi_result)
        ...
        ! [cite_start]Solves for chi using an iterative method. [cite: 10]
 ...
 * kappa / alpha < 3.0d0) then 
    ...
    function kappaplanck(opac, temp) result(kappa)
        !
        ! [cite_start]Calculates the Planck-averaged opacity. [cite: 18]
 ...
 = 0.0d0 
        ...
        do i = 2, nf - 1
            ...
            aa = aa + bnu(i) * opac%cabs(i, 1, 1) * dnu ! [cite_start]Assuming a single species [cite: 20]
            bb = bb + bnu(i) * dnu
        end do
        kappa = aa / bb
        return
    ...
    function solverinselfirr(rin0, Sigma0, rsig, plsigma, kap, Hp0, &
        ...
        ! [cite_start]Solves for the inner disk radius with self-irradiation. [cite: 21]
    ...
    function mass(rhodust, r, t, gastodust) result(total_mass)
        !
        ! [cite_start]Calculates the total disk mass from a 2D density grid. [cite: 28]
 ...
 = 2, nr 
        ...
        do ir = 1, nr
            ...
            do it = 1, nt 
                ...
                m_val = m_val + vol * rhodust(ir, it, 1) ! [cite_start]Assuming a single species [cite: 35]
            end do
        end do
    ...
    function readopac(nr) result(opac)
        !
        ! [cite_start]Reads a dust opacity file. [cite: 36]
        integer, intent(in), optional :: nr
        type(opacity_data_type) :: opac
        
        !
        ! [cite_start]Local variables [cite: 37]
        ...
        !
        ! [cite_start]Determine file number from optional argument. [cite: 38]
        if (present(nr)) then
            nr_val = nr
        else
            nr_val = 1
        ...
        !
        ! [cite_start]Construct the filename. [cite: 39]
        write(filename, '(A, I0, A)') 'dustopac_', nr_val, '.inp'
        write(*,*) 'Reading ', trim(filename)
        
        !
        ! [cite_start]Open the opacity file. [cite: 40]
        iunit = 10
        ...
        !
        ! [cite_start]Read format and number of species/frequencies. [cite: 41]
        read(iunit, *) nf_val, ns_val
        
        !
        ! [cite_start]Check the file format. [cite: 42]
        if (nf_val >= 1) then
            !
            ! [cite_start]Format 1: Simple absorption/scattering cross-section. [cite: 43]
            opac%nf = nf_val
            opac%ns = ns_val
            opac%nrt = 1
            ...
            ! Read absorption data. [cite_start]Loop order is now correct. [cite: 44]
            do k = 1, nf_val
                do is = 1, ns_val
                    read(iunit, *) dum
            ...
            !
            ! Read scattering data. [cite_start]Loop order is now correct. [cite: 46]
            do k = 1, nf_val
                do is = 1, ns_val
                    read(iunit, *) dum
            ...
            !
            ! [cite_start]Format 2: More complex, includes temperature ranges. [cite: 48]
            opac%nf = ns_val
            opac%ns = nf_val
            ...
            !
            ! [cite_start]Check for smoothing, which is not supported. [cite: 49]
            if (nint(a) /= 0) then
            ...
            ! [cite_start]Read temperature ranges and data. [cite: 51]
            do i = 1, opac%nrt 
                read(iunit, *) a, opac%trange(i+1)
                
                !
                ! Read absorption data. [cite_start]Loop order is now correct. [cite: 53]
                do k = 1, opac%nf
                    do is = 1, opac%ns
                        read(iunit, *) dum
                ...
                !
                ! Read scattering data. [cite_start]Loop order is now correct. [cite: 55]
                do k = 1, opac%nf
                    do is = 1, opac%ns
                        read(iunit, *) dum
        ...
        !
        ! [cite_start]Read the frequency file. [cite: 57]
    ...
    function maketau(a, kappa) result(tau_result)
        ...
        ! [cite_start]This function calculates the optical depth in the radial and vertical directions. [cite: 62]
        type(disk_data_type), intent(in) :: a 
        real(kind=8), intent(in), dimension(:) :: kappa
        type(optical_depth_type) :: tau_result
        
       
        ! [cite_start]Local variables [cite: 64]
        integer :: nr, nt, nspec, nspecc
        real(kind=8) :: dummy
        integer :: ir, it, ispec
        
       
        ! [cite_start]Get array sizes. [cite: 65]
        nr = size(a%rho, dim=1)
        nt = size(a%rho, dim=2)
        ...
        !
        ! [cite_start]Allocate the output arrays. [cite: 67]
        allocate(tau_result%taur(nr, nt), tau_result%taut(nr, nt))
        
        !
        ! [cite_start]--- Calculate radial optical depth (taur) --- [cite: 68]
        do it = 1, nt
        ...
        !
        ! [cite_start]--- Calculate vertical optical depth (taut) --- [cite: 71]
        do ir = 1, nr
    ...
    subroutine smooth_rim(r, theta, rhodust, kappa, sigdust, drsm, tautol)
        !
        ! [cite_start]This subroutine smoothes the inner rim of a disk if the optical depth is too high. [cite: 74]
        ...
        !
        ! [cite_start]Local variables [cite: 76]
        integer :: nr, nt, nspec
        integer :: ispec, ir, i
        ...
        !
        ! [cite_start]Get array sizes. [cite: 77]
        nr = size(r)
        nt = size(theta)
        nspec = size(rhodust, dim=3)
        
        !
        ! [cite_start]Set default values for optional arguments. [cite: 78]
        ...
        !
        ! [cite_start]--- Find the optical depth of the first cell at the midplane. [cite: 80]
        ! [cite_start]--- [cite: 81]
        tau = 0.0d0
        do ispec = 1, nspec
        ...
        !
        ! [cite_start]--- If this is too high, then smooth the inner rim a bit. [cite: 82]
        ! [cite_start]--- [cite: 83]
        if (tau > tautol_val) then
            !
            ! [cite_start]Calculate the required reduction factor. [cite: 84]
            rdxsm = tautol_val / tau
            
            write(*,*) 'Smoothing inner rim with a factor ', rdxsm
            
            !
            ! [cite_start]Make a smooth curve from 0 to 1. [cite: 85]
            ...
            !
            ! [cite_start]This is a simplified translation of the IDL `alog10`. [cite: 87]
            do i = 1, nr
            ...
            !
            ! [cite_start]Ensure `z_arr(1)` is 0.0d0. [cite: 89]
            z_val = z_arr(1)
            do i = 1, nr
            ...
            !
            ! [cite_start]Now reduce the density. [cite: 90]
            do ir = 1, nr
                x = 1.0d0 - z_arr(ir)
                redux = rdxsm**x
    ...
    function integrate(x, f, cons, prim) result(integral_result)
        !
        ! [cite_start]This function performs a numerical integration of a 1D array. [cite: 93]
       real(kind=8), intent(in), dimension(:) :: x, f 
        logical, intent(in), optional :: cons, prim
        real(kind=8), allocatable :: integral_result(:)
        
        !
        ! [cite_start]Local variables [cite: 95]
        integer :: n, i
      ...
      total_integral = 0.0d0
        ...
        if (present(cons)) then
            !
            ! [cite_start]Constant integration method [cite: 98]
            do i = 2, n
            ...
            !
            ! [cite_start]Trapezoidal integration method (default) [cite: 100]
            do i = 2, n
