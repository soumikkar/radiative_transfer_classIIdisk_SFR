...
program main_program
    ...
    use disk_radmc_inpfiles, only: write_star, write_radmc, write_vstruct, write_chopdens
    ...
    ! ============================================
    ! This part is based on the logic in the original problem_setup.pro
    ...
    call simpledisk_vertstruct( &
        ...
        kurucz_in=(params%kurucz == 1), kurdir_in=params%kuruczdir, &
        bindir_in=params%bindir, ifast_in=(params%ifast == 1), dostr_in=dostr)
    ...
    call write_star(params%mstar, params%rstar, params%tstar, &
                    (params%kurucz == 1), params%kuruczdir, inupk, lampk)

    write(*,*) 'Generating chopdens.inp...'
    ! Assuming these are not part of the main parameters for a simple run
    !call write_chopdens(10.0d0, 0.55d0, 1.0d0)
